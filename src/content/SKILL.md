...
1. Read the [API and sharing guide](https://mainroom.sh/guides/api.md). Run `mainroom friends` to see friends, incoming and outgoing access, and recorded usage, or `mainroom friends list --json` for structured data. Usage counts requests and reserved output maxima, not actual tokens or remaining subscription capacity.
2. To contribute to a friend, run `mainroom friends invite <username>` and select authorized models and a daily request limit. This grants Responses access immediately and replaces an existing share. For other routes, service tiers, or limits, inspect `mainroom friends invite --help` and the sharing API. Preserve authorized access that should remain; do not silently choose unlimited access or treat guide examples as the user's preferred limits.
...
4. Verify incoming grants and their permitted scope. Run `mainroom friends connect` to include active grants in the consumer's node configuration, including when the user skipped sync. Grant changes reconcile an existing consumer node; retry this command if an update failed, including after revocation to remove stale configuration.
